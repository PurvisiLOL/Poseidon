 ...
 * @returns {{score: number, closestString: string, minDistance: number}} An object containing the score
...
export function findClosestString(target, array) {
  try {
    ...
    return { score, closestString, minDistance };
  } catch (e) {
    console.log(e);
    throw new Error(
...
function computeLevenshteinDistance(a, b) {
  /**
   The Levenshtein algorithm finds the minimum number of edits (insertions, deletions, substitutions) needed to turn one word into another. 
   Imagine morphing one string into the other, counting the changes.
  */
...
export default findClosestString;
